...
import {
  createCourseVideoAwait,
  getCourseVideoByCourseId,
} from "../../modules/courseVideos/service.js";
...
export const courseVideoUpload = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "yahaaa!! New Course Created Successfully" });
  } catch (err) {
    console.log(err);
  }
};
...
export const getOneCourseVideos = async (req, res) => {
  ...
  try {
    const courseVideos = await getCourseVideoByCourseId({
      course_id: course_id,
    });
    if (courseVideos) {
